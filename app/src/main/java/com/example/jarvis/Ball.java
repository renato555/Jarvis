...
public class Ball {
    ...
    public void update( Player player1, Player player2, Score score){
        ...
        //bounce off player1
        float x1 = player1.getPositionX(); float y1 = player1.getPositionY();
        if(  ( positionX >= x1 - Player.width / 2 && positionX <= x1 + Player.width / 2) && ( positionY + radius >= y1 && positionY + radius <= y1 + Player.imaginaryHeight)){
            velocityY *= -1;
            velocityY *= velocityMultiplier; //speed up in y direction
        }
        //bounce off player2
        float x2 = player2.getPositionX(); float y2 = player2.getPositionY();
        if(  ( positionX >= x2 - Player.width / 2 && positionX <= x2 + Player.width / 2) && ( positionY - radius <= y2 + Player.height  && positionY - radius >= y2 + Player.height - Player.imaginaryHeight)){
            velocityY *= -1;
        }
